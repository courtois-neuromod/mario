...
These TSV files (`*_desc-annotated_events.tsv`) contain game events aligned to fMRI timing. They are generated by combining the original `*_events.tsv` (which contains repetition timing) with hardcoded interpretations of the `*_variables.json` files (see `code/annotations/`). Each row represents either a **repetition** (gameplay attempt) or a **game event** detected within that repetition.

**Columns:**
| Column | Description |
|--------|-------------|
| `trial_type` | Event type (see below) |
| `rep_index` | Repetition index within the run |
| `level` | Level identifier (e.g., w1l1) |
| `onset` | Time in seconds from start of fMRI run |
| `duration` | Event duration in seconds |
| `frame_start` / `frame_stop` | Frame indices in the replay |
| `phase` | Experimental phase (discovery/practice) — only for `gym-retro_game` rows |
| `index` | Repetition index within the run — only for `gym-retro_game` rows |
| `stim_file` | Path to corresponding .bk2 file — only for `gym-retro_game` rows |

**Event types (`trial_type`):**
- `gym-retro_game`: A gameplay repetition (attempt at a level)
- **Button presses**: `UP`, `DOWN`, `LEFT`, `RIGHT`, `JUMP`, `RUN/THROW`
- **Kills**: `Kill/stomp`, `Kill/impact` (shell/fireball), `Kill/kick`
- **Hits taken**: `Hit/fall` (life lost by falling), `Hit/killed` (life lost by enemy), `Hit/powerup_lost` (damage taken)
- **Collectibles**: `Coin_collected`, `Powerup_collected`, `Brick_smashed`
- **Level events**: `Level_complete`
